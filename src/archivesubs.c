...
/* This will force a char into the binaries , strings can then
be uesd to tell that this code is linked in   
 */

static const char *svn_id __attribute__ ((unused)) = "$Id$";
...
/* Given a filename with path, do the equivalent of mkdir -p */
/* Assumes filename included unless ends in /  */
/* Original code provided by Michelle Gower */
int mkpath(filename,flag_verbose) 
	const char *filename;
	int flag_verbose;
{
  char event[10000];
    ...
    /* Initialize partialpath to empty */
    partialpath = (char *) malloc(len * sizeof(char));
    if (!partialpath) {
        if (flag_verbose >= 2) fprintf(stderr,"STATUS5BEG\n");
        fprintf(stderr,"Could not malloc partialpath %d\n", len);
        if (flag_verbose >= 2) fprintf(stderr,"STATUS5END\n");
        exit(1);
    }
    partialpath[0] = 0;

    if (filename[0] == '/') {
        /* handle case of absolute path */
        strcpy(partialpath, "/");
        posL++;
    }
    else if (filename[0] == '!') {
        /* ignore leading ! that is used for FITS files */
        posL++;
    }

    posR = posL;
    while (posR < len) {
        while ((posR < len) && (filename[posR] != '/')) posR++;
        if (posR < len) {
            strncat(partialpath, &(filename[posL]), posR - posL);
            strcat(partialpath,"/");

            /* check if partialpath exists */
            if(stat(partialpath,&st) != 0) {
                /* call C's mkdir function to create directory */
                save_errno = 1;
                tries = 1;
		  ...
		  if (mkdir(partialpath, 0770) < 0) {
		    ...
		    if (save_errno == EEXIST) {
		      ...
		      if (tries >= maxtries) {
			sprintf(event,"Failed to create path: %s (errno=%d). Aborting",partialpath,save_errno);
			reportevt(flag_verbose,STATUS,5,event);
			break;
		      }
		      else {
			sprintf(event,"Failed to create path: %s (errno=%d). Retrying",partialpath,save_errno);
...
/* pull everything from Archive/ and above off a name */
char *striparchiveroot(filename)
	char filename[];
{
	int i;
	...
	i=strlen(filename);
	while(i>=0) {
	  if (!strncmp(filename+(--i),"Archive/",8)) return(filename+i+8);
	}
	/* No root found.  Return full filename */
	return(filename);
}
